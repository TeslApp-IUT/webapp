...
final class RegistrationController
{
  ...
  public function create(): void
  {
    ...
    try {
      ...
      error_log(
        '[RegistrationController::create] Failed to fetch specializations: ' . $e->getMessage(),
      );
      // Ajout d'un message d'erreur utilisateur
      $errors = array_filter(
        array_merge((array) $errors, ['Impossible de charger la liste des spécialisations.']),
      );
      $specializations = [];
    }
  ...
  public function store(): void
  {
    ...
    if (!RateLimit::check('register', maxAttempts: 10, windowSeconds: 3600)) {
      ...
      Flash::set('errors', [
        "Trop de tentatives de création de compte. Réessayez dans {$minutes} minutes.",
      ]);
    ...
    if ($msg = Inputs::validateName($firstname, max: 32, label: 'Le prénom')) {
      $errors[] = $msg;
    }
    if ($msg = Inputs::validateName($lastname, max: 32, label: 'Le nom')) {
      $errors[] = $msg;
    }
    if ($msg = Inputs::validateUsername($username, min: 3, max: 32)) {
      $errors[] = $msg;
    }
    if ($msg = Inputs::validateEmail($email, max: 254)) {
      $errors[] = $msg;
    }
    ...
    if ($msg = Inputs::validatePasswordConfirmation($password, $password2)) {
      $errors[] = $msg;
    }
    ...
    if ($hash === false || ($msg = Inputs::validateArgon2idPHC($hash))) {
      if ($hash === false) {
        error_log('[Register] password_hash returned false');
      }
    ...
    try {
      ...
      if ($e->getCode() === '23505') {
        ...
        if (stripos($msg, 'username') !== false) {
          $human = 'Ce nom d\'utilisateur est déjà utilisé.';
        }
        if (stripos($msg, 'email') !== false) {
          $human = 'Cet email est déjà utilisé.';
        }
